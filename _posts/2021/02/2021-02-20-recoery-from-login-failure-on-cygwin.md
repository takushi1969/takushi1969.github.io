---
layout: post
title:  can't invoke mintty of cygwin after making cygserver run at boot.
author: takushi1969
...
[Cygwin](https://cygwin.com/)はPosix機能の以外に
...
[Cygserver](https://cygwin.com/cygwin-ug-net/using-cygserver.html)は通
常であれば、Windowsのブート時に自動起動するよう設定する。
...
しかし、会社などの組織で運用しているActive DirectoryなどのDomainに属しているWindowsで[Cygserver](https://cygwin.com/cygwin-ug-net/using-cygserver.html)をブート起動した場合、Windowsのログインに使用するドメイン・ユーザのアカウント・キャシュされない。このため、Cygwin Terminalを起動に失敗する。
...
### 1. Cygserverの停止 ###
  ...
  2. stop-serviceコマンドレットの引数にCygserverを指定いて実行
	 ![stop cygserver](/assets/2021/02/20/stop-cygserver.gif)
  3. Windowsの再起動
 
### 2. /etc/passwd、/etc/groupを作成 ###

上記手順の結果、Cygwin Terminalが起動可能となった。
...
上記の反映を終えたら、システムの再起動を行う。

これは、[The /etc/nsswitch.conf file](https://cygwin.com/cygwin-ug-net/ntsec.html#ntsec-mapping-nsswitch)にある以下の注記に対応して、/etc/nsswitch.confの変更が有効するためである。また、手順1で停止したCygserverを自動起動するためでもある。
